...
class ContextPackBuilderTest < Minitest::Test
  def test_build_includes_skill_and_backtrace_snippet
    test_file = File.join(Dir.pwd, "tmp", "patchcaptain_context_builder_test.rb")
    ...
    result = PatchCaptain::ContextPackBuilder.new(payload, configuration: config).build
  ...
  def test_build_reads_files_from_context_directory_targets
    root = Dir.pwd
    dir = File.join(root, "tmp", "patchcaptain_context_dir")
    ...
    assert_includes result, "Project File: tmp/patchcaptain_context_dir/instructions.md"
